...
/*
static uint32_t get_fpu_tag_word() {
  uint32_t fptag = 0;
  unsigned int i;
  ...
  for (i=7; i>=0; i--) {
    fptag <<= 2;
    if (DECAF_cpu_fptags[i]) {
      fptag |= 3;
    }
    else {
      cpu_get_fp80(&float_significand,&float_exponent,DECAF_cpu_fp_regs[i].d);
...
void decode_address(uint32_t address, EntryHeader *eh, int ignore_taint)
{
  ...
  DECAF_read_mem(NULL, address, MAX_INSN_BYTES, insn_buf);
  ...
  */
  ...
  if (tracepid != -1) {
    ...
    eh->pid = find_pid(DECAF_cpu_cr[3]);
  }
        ...
        if (-1 == regnum)
          ...
          switch (eh->operand[op_idx].addr.reg_addr) {
            ...
            case edi_reg:
              ...
              eh->operand[op_idx].value.val32 = DECAF_cpu_regs[regnum];
              break;
            ...
            case gs_reg:
              ...
              eh->operand[op_idx].usage = unknown;
              eh->operand[op_idx].value.val32 = 
                DECAF_cpu_segs[regnum].selector;
              eh->operand[op_idx].value.val32 &= 0xFFFF;
              break;
            /* MMX registers */
            case mmx0_reg:
            ...
            case mmx7_reg:
              ...
              eh->operand[op_idx].value.val64 = 
                DECAF_cpu_fp_regs[regnum].mmx.q;
              break;
            case xmm0_reg:
            case xmm1_reg:
            case xmm2_reg:
            ...
            case xmm15_reg:
              ...
              eh->operand[op_idx].value.xmm_val._q[0] = 
                DECAF_cpu_xmm_regs[regnum]._q[0];
              eh->operand[op_idx].value.xmm_val._q[1] = 
                DECAF_cpu_xmm_regs[regnum]._q[1];
              break;
            ...
            case fpu_status_reg:
              ...
              */
              ...
              eh->operand[op_idx].usage = unknown;
              eh->operand[op_idx].value.val32 =
                ((uint32_t) *DECAF_cpu_fpus & 0xC7FF) |
                (((uint32_t) *DECAF_cpu_fp_stt & 0x7) << 11);
            ...
            case fpu_control_reg:
              ...
              eh->operand[op_idx].type = TFloatControlRegister;
              eh->operand[op_idx].usage = unknown;
              eh->operand[op_idx].value.val32 = (uint32_t) *DECAF_cpu_fpuc;
              // The following should not be needed. Check if bug in XED
              eh->operand[op_idx].length = 2;
              break;
            /* EFLAGS register */
        ...
        for (j = 0; j < memlen; j+=4) {
          ...
          if (seg_regid != XED_REG_INVALID) {
            ...
            if (!default_segment) {
              ...
              DECAF_read_mem(NULL, segent, 8, segdes);
          ...
          if (op_name == XED_OPERAND_AGEN) {
            ...
            eh->operand[op_idx].length = 4;
              ...
              DECAF_read_mem(NULL, eh->operand[op_idx].addr.mem32_addr,
...
int close_trace(FILE *stream) {
  ...
  procinfo_t proc;
  uint32_t num_procs = 0, num_found_processes = 0;
  ProcRecord pr;
  unsigned int i, j;
  uint32_t num_elems_written = 0;
  size_t modlist_size = 0;
  uint32_t trailer_begin = TRAILER_BEGIN;
  uint32_t trailer_end = TRAILER_END;
  uint32_t process_list_size = 0;

  // Get array of information about tracked processes
  proc_arr = trackproc_get_tracked_processes_info(tracepid, &num_procs);
  ...
  if (proc_arr) {
    ...
    for (i = 0; i < num_procs; i++) {
      ...
      pr.pid = proc.pid;
      pr.n_mods = proc.n_mods;
      strncpy(pr.name, proc.name, MAX_STRING_LEN - 1);
      pr.name[MAX_STRING_LEN - 1] = '\0';

      modlist_size = pr.n_mods * sizeof(old_modinfo_t);

      if (modlist_size > 0) {
        old_modinfo_t *pmr = (old_modinfo_t *) malloc(modlist_size);
        if (pmr) {
          get_proc_modules(pr.pid, pmr, pr.n_mods);
        }
        else {
        ...
        num_elems_written += fwrite(&pr, sizeof(pr), 1, stream);
        process_list_size+=sizeof(pr);

        if (pmr) {
          ModuleRecord mr;
          for (j = 0; j < pr.n_mods; j++) {
            strncpy(mr.name, pmr[j].name, MAX_STRING_LEN);
            mr.base = pmr[j].base;
            mr.size = pmr[j].size;
            num_elems_written += fwrite(&mr, sizeof(mr), 1, stream);
            process_list_size+=sizeof(mr);
          }
          free(pmr);
        }
      }
