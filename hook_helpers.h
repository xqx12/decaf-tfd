...
{
  return (DECAF_read_mem(NULL, vaddr, length, buf) < 0);
}

static inline int write_mem(uint32_t vaddr, int length, unsigned char *buf)
{
  return (DECAF_write_mem(NULL, vaddr, length, buf) < 0);
}

static inline void read_reg(int reg_id, uint32_t *val)
{
  *val = 0;
  DECAF_read_register(reg_id, val);
}

static inline void write_reg(int reg_id, uint32_t val)
{
  DECAF_write_register(reg_id, &val);
}
