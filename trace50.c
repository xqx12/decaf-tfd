...
/* Return the offset of the operand. Zero except for AH,BH,CH,DH where one */
      ...
      case XED_OPERAND_REG15: {
        ...
        if (-1 == regnum)
          ...
          switch (eh->operand[op_idx].addr) {
            ...
            case ch_reg:
            case dh_reg:
              eh->operand[op_idx].value = 
                (eh->operand[i].value & 0xFF00) >> 8;
              break;
            default:
              break;
          }
        ...
        for (j = 0; j < memlen; j+=4) {
          ...
          if (seg_regid != XED_REG_INVALID) {
            ...
            if (!default_segment) {
              ...
              eh->memregs[op_idx][3].value = *segdes_ptr;
          ...
          if (op_name == XED_OPERAND_AGEN) {
            ...
            has_page_fault = 
              DECAF_read_mem(cpu_single_env,eh->operand[op_idx].addr,
...
unsigned int write_insn(FILE *stream, EntryHeader *eh)
{
  ...
  if (stream && (eh->inst_size > 0)) {
    ...
    insninfostr[0] = 0;
    checkcr3(DECAF_plugin->monitored_cr3, savedeip, tracepid, name,
