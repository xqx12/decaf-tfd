...
void malloc_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog, 
      "%08lld %08lld %04u %s::%s ALLOC 0x%08x %d %d 0x0 0 0x0 0 0x%08x\n",
...
void calloc_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog, 
      "%08lld %08lld %04u %s::%s CALLOC 0x%08x %d %d 0x0 0 0x0 0 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name, 
      fun_name, eax, s->size, sizeT, eip);
  }
...
void memalign_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s ALLOC 0x%08x %d 0 0x0 0 0x0 0 0x%08x\n",
...
void free_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog, "%08lld %08lld %04u %s::%s FREE 0x%08x 0x%08x\n", 
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name, 
...
void realloc_ret(void *opaque)
{
  ...
  if (alloclog) {
    int sizeT = 0;
    fprintf(alloclog, "%08lld %08lld %04u %s::%s FREE-R 0x%08x 0x%08x\n", 
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name, 
      fun_name, s->bufPtr, eip);
    /* If new size is zero, realloc behaves as free, so do not print alloc */
    if (s->bufSize > 0) {
      fprintf(alloclog, 
        "%08lld %08lld %04u %s::%s ALLOC-R 0x%08x %d %d 0x0 0 0x0 0 0x%08x\n",
        s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name, 
...
void heap_create_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s NEWHEAP 0x%08x %d 0x%08x 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name,
...
void rtl_create_heap_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s NEWHEAP 0x%08x %d 0x%08x %d 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name,
...
void rtl_allocate_heap_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog, 
      "%08lld %08lld %04u %s::%s ALLOC 0x%08x %d %d "
...
void rtl_reallocate_heap_ret(void *opaque)
{
  ...
  if (alloclog) {
    ...
    fprintf(alloclog, 
      "%08lld %08lld %04u %s::%s ALLOC-R 0x%08x %d %d "
      "0x%08x %d 0x%08x %d 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name, 
      fun_name, eax, s->size, sizeT, s->heapHandle, handleT, s->flags, 
      flagsT, eip);
...
void zw_map_view_of_section_ret(void *opaque)
{
  ...
  if (alloclog) {
    /* NOTE: The allocation site actually encodes s->baseAddressPtr */
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s MAP 0x%08x %d 0 0x0 0 0x0 0 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, 
...
void zw_unmap_view_of_section_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog, "%08lld %08lld %04u %s::%s UNMAP 0x%08x 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, 
...
void virtual_alloc_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s VALLOC 0x%08x %d 0x%08x 0x%x 0x%x 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, 
...
void virtual_free_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog, 
      "%08lld %08lld %04u %s::%s VFREE 0x%08x %d 0x%x 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, 
...
void local_alloc_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s %s 0x%08x %d 0 0x0 0 0x0 0 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, mod_name, 
...
void local_realloc_ret(void *opaque)
{
  ...
  if (alloclog) {
    ...
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s ALLOC-R 0x%08x %d 0 0x0 0 0x0 0 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, 
      mod_name, fun_name, eax, s->size, eip);
  }
...
void cf_allocator_allocate_ret(void *opaque)
{
  ...
  if (alloclog) {
    fprintf(alloclog,
      "%08lld %08lld %04u %s::%s ALLOC 0x%08x %d 0 0x%08x 0 0x%08x 0 0x%08x\n",
      s->insn_ctr, tstats.insn_counter_traced, s->entry_tid, 
